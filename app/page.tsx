...
type Role = "judge"|"appeal_judge"|"plaintiff"|"defendant"|"victim"|"plaintiff_attorney"|"defense_attorney"|"public_defender"|"prosecutor"|"prosecutor_assistant"|"legal_researcher"|"witness"|"expert"|"technical_assistant"|"juror"|"clerk"|"bailiff";
const roles:{id:Role;title:string;description:string}[] = [
 ...
 {id:"public_defender",title:"Defensor Público",description:"Atue pela Defensoria Pública quando cabível."},
 {id:"prosecutor",title:"Promotor de Justiça",description:"Atue pelo Ministério Público quando cabível."},
 {id:"prosecutor_assistant",title:"Assistente de Acusação",description:"Atue na assistência à acusação quando cabível."},
 {id:"victim",title:"Vítima",description:"Atue diretamente como vítima do fato simulado."},
 ...
 {id:"technical_assistant",title:"Assistente Técnico",description:"Atue na análise técnica e formule esclarecimentos."},
 {id:"judge",title:"Juiz de Direito",description:"Conduza a audiência e profira decisões."},
 {id:"appeal_judge",title:"Desembargador",description:"Atue no julgamento em segunda instância."},
 {id:"juror",title:"Jurado",description:"Participe do conselho de sentença no júri."},
 {id:"clerk",title:"Servidor da Secretaria",description:"Pratique atos formais e registros da audiência."},
 {id:"bailiff",title:"Oficial de Justiça",description:"Atue no cumprimento dos atos judiciais."},
...
type ProcessData = {number?:string;area?:string;case_type?:string;plaintiff?:string;defendant?:string;facts?:string;include_mp?:boolean;jury?:boolean;status?:string};
...
export default function Home(){
 ...
 async function enterCourtroom(){
   if(!userName.trim()){setError("Informe seu nome para entrar no julgamento.");return;} rememberName(userName.trim());setLoading(true);setError("");
 ...
 function leaveCourtroom(){localStorage.removeItem(SESSION_STORAGE_KEY);setInChat(false);setStarted(false);setPanel(null);setMessages([]);setChatText("");setIdentity(null);setError("");setProcessId("");setProcessNumber("");setCharacters([]);setProcessData(null);setPhase("opening");}
 ...
 const details=caseSections(processData?.facts||"");
 ...
 :<section className="courtroom"><div className="court-head"><div><div className="eyebrow">SALA DE JULGAMENTO · 1ª INSTÂNCIA</div><h2>Processo nº {processNumber}</h2><p><b>{identity?.display_name||userName||roleName}</b> · {roleName} · {areaName} · fase: {phases[Math.min(phaseIndex,phases.length-1)]}</p></div><button className="secondary-action" onClick={leaveCourtroom} disabled={agentLoading}><LogOut size={16}/> Sair do julgamento</button></div><div className="court-tools"><button onClick={()=>setPanel(panel==="details"?null:"details")}><FileText size={16}/> Processo</button><button onClick={loadParticipants}><Users size={16}/> Participantes</button><button onClick={()=>setPanel(panel==="documents"?null:"documents")}><Landmark size={16}/> Audiência</button><button onClick={()=>setPanel(panel==="appeals"?null:"appeals")}><GitBranch size={16}/> Recursos e instâncias</button></div>{panel&&<aside className="court-panel">
 {panel==="details"&&<><h3>Detalhes do caso</h3><p><b>Processo:</b> {processNumber}</p><p><b>Área:</b> {areaName}</p><p><b>Classe/tema:</b> {details.title}</p><p><b>Autor:</b> {processData?.plaintiff||"Não informado"}</p><p><b>Réu:</b> {processData?.defendant||"Não informado"}</p><p><b>Instância:</b> 1ª instância</p><p><b>Seu papel:</b> {identity?.display_name||userName} · {roleName}</p><hr/><h4>Resumo fático</h4><div className="message-content" dangerouslySetInnerHTML={{__html:richContent(details.narrative)}}/><h4>Questão central</h4><div className="message-content" dangerouslySetInnerHTML={{__html:richContent(details.issue)}}/><h4>Provas indicadas</h4>{details.evidence.length?<ul>{details.evidence.map((x,i)=><li key={i}>{x}</li>)}</ul>:<p>Não especificadas.</p>}<h4>Procedimento</h4><p>{details.procedure}</p><p><b>Ministério Público:</b> {processData?.include_mp?"Atuação habilitada":"Não habilitado"}</p><p><b>Tribunal do Júri:</b> {processData?.jury?"Sim":"Não"}</p></>}
 {panel==="people"&&<><h3>Pessoas no julgamento</h3>{characters.map((c,i)=>{const mine=c.name===identity?.display_name;return <div className="person" key={c.id||i}><b>{c.title} {c.name}{mine?" · Você":""}</b><small>{c.profession||"Participante fictício"}</small></div>})}</>}
 {panel==="documents"&&<><h3>Condução da audiência</h3><div className="person"><b>Debate livre</b><small>Você pode se manifestar a qualquer momento. O juízo controla a ordem e a pertinência.</small></div><div className="person"><b>Próximo ato</b><small>Use o botão Continuar julgamento junto ao campo de manifestação para solicitar a próxima fala prevista.</small></div></>}
 {panel==="appeals"&&<><h3>Recursos e instâncias</h3><p>Depois de uma decisão recorrível, a parte legitimada poderá protocolar o recurso cabível.</p><p><b>Instância atual:</b> 1ª instância</p><p><b>Fluxo:</b> 1ª instância → 2ª instância → STJ → STF, conforme cabimento.</p><button className="secondary-action">Preparar recurso</button></>}
 </aside>}
 <div className="phase-strip">{phases.map((p,i)=><span className={i<=phaseIndex?"active":""} key={p}>{p}</span>)}</div><div className="chat-window"><div className="chat-title"><span><MessageSquare size={17}/><b>Plenário virtual</b></span><small>{agentLoading?<><Bot size={13}/> IA em manifestação</>:"Debate livre · controle processual"}</small></div><div className="messages">{messages.map((m:any,i:number)=><div className={m.kind==="user"?"message user":"message"} key={m.id||i}><small>{m.sender||m.actor}</small><div className="message-content" dangerouslySetInnerHTML={{__html:richContent(m.content||"")}} /></div>)}</div><div className="composer"><textarea value={chatText} onChange={e=>setChatText(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();sendMessage()}}} placeholder="Digite sua manifestação, pergunta ou intervenção..." rows={2}/><div style={{display:"flex",gap:8,justifyContent:"flex-end",flexWrap:"wrap"}}><button onClick={sendMessage} disabled={!chatText.trim()||agentLoading}><Send size={17}/> Enviar</button><button className="secondary-action" onClick={nextSpeaker} disabled={agentLoading||loading}><Play size={16}/>{agentLoading?" IA em manifestação...":" Continuar julgamento"}</button></div></div></div>{error&&<div className="error">{error}</div>}<div className="notice"><Landmark size={15}/><span>O debate permanece livre. <b>Continuar julgamento</b> solicita apenas a próxima fala/ato previsto; você pode pressioná-lo novamente sempre que quiser que o processo avance. Ele não troca de fase por si só.</span></div></section>}
 </main>;
}
