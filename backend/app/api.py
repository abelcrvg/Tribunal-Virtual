...
def create_new_process(data: ProcessCreate, db: Session = Depends(get_db)) -> Process:
    ...
    process = create_process_db(db, data, number)
    return process
...
def analyze_process_appeal(process_id: UUID, data: AppealRequest, db: Session = Depends(get_db)):
    ...
    if result.admissible:
        get_case_memory(str(process_id)).record(ProcessEvent(type="appeal", actor=data.appellant_role, content=f"{result.type.value}: {result.reason}", relevance="pertinent"))
