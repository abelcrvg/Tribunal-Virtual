...
class ChatRequest(BaseModel): role:str=Field(min_length=2,max_length=80); turn_role:str=Field(min_length=2,max_length=80); actor:str=Field(min_length=2,max_length=120); content:str=Field(min_length=1,max_length=10000)
class AppealRequest(BaseModel): case_area:str=Field(min_length=2,max_length=40); decision_type:str=Field(min_length=2,max_length=80); appellant_role:str=Field(min_length=2,max_length=80); appeal_type:AppealType
...
def record_process_event(process_id:UUID,data:EventRequest,db:Session=Depends(get_db)):
 _get_process(process_id,db); get_case_memory(str(process_id)).record(ProcessEvent(type=data.type,actor=data.actor,content=data.content,relevance=data.relevance)); event=store.add(str(process_id),data.type,data.actor,data.content,data.relevance); return {"id":event.id,"recorded":True,"relevance":event.relevance}
@router.post("/{process_id}/chat",tags=["courtroom"])
def courtroom_chat(process_id:UUID,data:ChatRequest,db:Session=Depends(get_db)):
 _get_process(process_id,db); message=submit_message(process_id=str(process_id),role=data.role,turn_role=data.turn_role,actor=data.actor,content=data.content); decision=assess_intervention(role=data.role,turn_role=data.turn_role,content=data.content); return {"id":message.id,"accepted":message.accepted,"assessment":message.assessment,"actor":message.actor,"content":message.content,"judge_response":decision.judge_response,"requires_record":decision.requires_record,"reason":decision.reason,"created_at":message.created_at}
...
def courtroom_intervention(process_id:UUID,data:InterventionRequest,db:Session=Depends(get_db)):
 _get_process(process_id,db); decision=assess_intervention(role=data.role,turn_role=data.turn_role,content=data.content)
 if decision.requires_record: get_case_memory(str(process_id)).record(ProcessEvent(type="intervention",actor=data.role,content=data.content,relevance=decision.assessment.value)); store.add(str(process_id),"intervention",data.role,data.content,decision.assessment.value)
...
def analyze_process_appeal(process_id:UUID,data:AppealRequest,db:Session=Depends(get_db)):
 _get_process(process_id,db); result=analyze_appeal(case_area=data.case_area,decision_type=data.decision_type,appellant_role=data.appellant_role,appeal_type=data.appeal_type)
 if result.admissible: store.add(str(process_id),"appeal",data.appellant_role,f"{result.type.value}: {result.reason}","pertinent")
...
def plaintiff(process_id:UUID,db:Session=Depends(get_db)):
 ...
 except RuntimeError as exc:raise HTTPException(503,detail=str(exc)) from exc
@router.post("/{process_id}/agents/defense",tags=["simulation"])
def defense(process_id:UUID,db:Session=Depends(get_db)):
 try: process=_get_process(process_id,db); p=run_plaintiff_agent(process); return run_defense_agent(process,p["content"])
 except RuntimeError as exc:raise HTTPException(503,detail=str(exc)) from exc
@router.post("/{process_id}/agents/research",tags=["simulation"])
def research(process_id:UUID,db:Session=Depends(get_db)):
 try: process=_get_process(process_id,db); p=run_plaintiff_agent(process); d=run_defense_agent(process,p["content"]); return run_research_agent(process,p["content"],d["content"])
 except RuntimeError as exc:raise HTTPException(503,detail=str(exc)) from exc
@router.post("/{process_id}/agents/judge",tags=["simulation"])
def judge(process_id:UUID,db:Session=Depends(get_db)):
 try: process=_get_process(process_id,db); p=run_plaintiff_agent(process); d=run_defense_agent(process,p["content"]); r=run_research_agent(process,p["content"],d["content"]); return run_judge_agent(process,p["content"],d["content"],r["content"])
 except RuntimeError as exc:raise HTTPException(503,detail=str(exc)) from exc
