...
def create_session(process_id:UUID,data:SessionCreate,db:Session=Depends(get_db)):
    p=_process(process_id,db); key=f"{process_id}:{data.role.value}"; s=CourtSession(id=key,process_id=str(process_id),user_role=data.role); _identities[key]=build_user_identity(data.user_id,data.role)
...
def automatic_agent_turn(process_id:UUID,role:UserRole,db:Session=Depends(get_db)):
    s=_session(process_id,role); p=_process(process_id,db)
