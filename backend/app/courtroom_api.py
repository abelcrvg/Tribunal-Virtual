...
def _restore_session(pid:UUID,role:UserRole,db:Session)->CourtSession|None:
    ...
    for event in events:
        if event.event_type=="phase_change":
            try: s.phase=CourtPhase(event.content.rsplit(": ",1)[-1].rstrip("."))
...
def _session(pid:UUID,role:UserRole,db:Session):
    ...
    if s is None:
        ...
        if s is not None:
            _sessions[key]=s
            locked=db.get(ProcessSessionDB,str(pid))
            saved_user_id=locked.user_id if locked is not None and locked.user_id else "local-user"
            _identities[key]=build_user_identity(saved_user_id,role)
...
def advance_phase(process_id:UUID,role:UserRole,db:Session=Depends(get_db)):
    ...
    s.phase=order[i+1]; s.turn_index=0; content=f"Passamos à fase: {s.phase.value}."; store.add(str(process_id),"phase_change","Magistrado",content,"pertinent"); m=s.add_message("Magistrado",MessageKind.RULING,content,UserRole.JUDGE); _persist_message(db,str(process_id),m); return {"phase":s.phase,"allowed_roles":list(s.allowed_roles),"message":m}
