...
def _agent_identity(process: Process, agent: str) -> str:
    # Identidades são fictícias e determinísticas para a simulação; não são dados reais.
...
def run_agent(process: Process, agent: str, name: str, role: str, system: str, instructions: str, provider: AIProvider | None = None):
    ...
    safety = (
        "Esta é uma simulação educacional. Todos os nomes, números de OAB, datas, locais e fatos não presentes nos autos "
        "devem ser fictícios. NUNCA use placeholders entre colchetes como [Cidade/UF], [Data], [Nome do Perito], "
        "[Nome do Advogado], OAB/[UF] ou [Número]. Use a identidade fictícia fornecida. "
        "Não escreva marcadores técnicos como CONCEDER_PALAVRA, REGISTRAR, irrelevant, pertinent, decisive ou abusive na manifestação. "
        "Não antecipe outra fase processual. Quando redigir uma peça, use Markdown ou HTML válido, sem barras invertidas antes de títulos."
    )
